  ...
  describe('timerSequenceManagement', () => {
    describe('addTimerToSequence', () => {
      const initialSequence: Timer[] = [
        { id: 'timer1', expectedTime: 30, elapsedTime: 0, isRunning: false, isOverrun: false },
        { id: 'timer2', expectedTime: 60, elapsedTime: 0, isRunning: false, isOverrun: false },
      ];
      const newTimer: Timer = { id: 'timer3', expectedTime: 45, elapsedTime: 0, isRunning: false, isOverrun: false };
    ...
    describe('removeTimerFromSequence', () => {
      const initialSequence: Timer[] = [
        { id: 'timer1', expectedTime: 30, elapsedTime: 0, isRunning: false, isOverrun: false },
        { id: 'timer2', expectedTime: 60, elapsedTime: 0, isRunning: false, isOverrun: false },
        { id: 'timer3', expectedTime: 45, elapsedTime: 0, isRunning: false, isOverrun: false },
      ];
  
      it('should remove a timer with the given ID', () => {
        const updatedSequence = removeTimerFromSequence(initialSequence, 'timer2');
        expect(updatedSequence.length).toBe(2);
        expect(updatedSequence.find(timer => timer.id === 'timer2')).toBeUndefined(); // timer2 is removed
      ...
      it('should return the original sequence if timerId is not found', () => {
        const updatedSequence = removeTimerFromSequence(initialSequence, 'timer4'); // ID not in sequence
    ...
    describe('moveTimerInSequence', () => {
      ...
      it('should return the original sequence if newPosition is invalid (out of bounds)', () => {
        const updatedSequence1 = moveTimerInSequence(initialSequence, 'timer2', -1); // Negative position
        const updatedSequence2 = moveTimerInSequence(initialSequence, 'timer2', 3);  // Position >= sequence length
    ...
    describe('reorderTimersInSequence', () => {
      ...
      it('should reorder timers according to the provided order of IDs', () => {
        const updatedSequence = reorderTimersInSequence(initialSequence, timerIdsInNewOrder);
      ...
      it('should return the original sequence if timerIdsInNewOrder has incorrect length', () => {
        ...
        const updatedSequence = reorderTimersInSequence(initialSequence, invalidOrder);
        expect(updatedSequence).toBe(initialSequence);
      });
